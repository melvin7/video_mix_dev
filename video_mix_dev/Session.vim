...
silent only
cd ~/dev/video_mix_dev/video_mix_dev
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +7 broadcastingstation.cpp
badd +5 encode_mux.h
badd +0 filter.h
args broadcastingstation.cpp
edit filter.h
...
exe 'vert 1resize ' . ((&columns * 21 + 63) / 127)
exe 'vert 2resize ' . ((&columns * 74 + 63) / 127)
exe 'vert 3resize ' . ((&columns * 30 + 63) / 127)
argglobal
enew
file NERD_tree_1
let s:cpo_save=&cpo
...
setlocal spellfile=
setlocal spelllang=en
setlocal statusline=%#airline_a_inactive#\ Tagbar\ %#airline_a_to_airline_b_inactive#%#airline_b_inactive#\ Name\ %#airline_b_to_airline_c_inactive#%#airline_c_inactive#\ filter.h\ 
setlocal suffixesadd=
setlocal noswapfile
setlocal synmaxcol=3000
if &syntax != 'tagbar'
