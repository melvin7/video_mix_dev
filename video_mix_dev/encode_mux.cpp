...
int open_output_file(OutputFile* of)
{
    ...
    if (fmt->video_codec != AV_CODEC_ID_NONE) {
        ...
        add_stream(&of->video_st, oc, &video_codec, fmt->video_codec);
        have_video = 1;
        encode_video = 1;
    }
    if (fmt->audio_codec != AV_CODEC_ID_NONE) {
        fmt->audio_codec = AV_CODEC_ID_AAC;
        add_stream(&of->audio_st, oc, &audio_codec, fmt->audio_codec);
        have_audio = 1;
        encode_audio = 1;
    }
    ...
    if (have_video)
        open_video(oc, video_codec, &of->video_st, NULL);

    if (have_audio)
        open_audio(oc, audio_codec, &of->audio_st, NULL);

    av_dump_format(oc, 0, of->filename, 1);
    int ret;
    /* open the output file, if needed */
    if (!(fmt->flags & AVFMT_NOFILE)) {
        ret = avio_open(&oc->pb, of->filename, AVIO_FLAG_WRITE);
        if (ret < 0) {
